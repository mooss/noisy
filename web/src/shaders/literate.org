...
The initial =diffuseColor= is multiplied by the color extracted from the color palette as a way to apply the texture.
Textures are not supported by every geometry style, so the =u_hasTexture= uniform decides whether to just use a flat color.
