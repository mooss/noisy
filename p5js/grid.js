...
function setup() {
    frameRate(10); // Avoids flickering.
    createCanvas(720, 400);

    // Calculate columns and rows.
    ncolumns = floor(width / cellSize);
...
// Fill board with a rainbow pattern, starting with white and ending with black.
function rainboard() {
    console.log(ncells);
    for (let column = 0; column < ncolumns; column++) {
        for (let row = 0; row < nrows; row++) {
            let ratio = (row * ncolumns + column) / ncells;
            currentCells[column][row] = interpolate(wrainbowb, ratio);
        }
    }
}
