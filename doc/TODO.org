...
*** TODO Integrate together the UI with the footer added by mooss.github.io
Or even better create the footer in noisy itself.
*** TODO Twitter icon in footer
*** TODO Harmonize the names of all root elements in GUI code
All the =_elt= and similar should be renamed to =container= (will require renaming the other things already named =container=).
*** TODO Fix the bug where the texture does not load when changing from Boxes to Surface
*** TODO Remove sampling parameters from layered noise (always use the default)
...
#+RESULTS:
#+begin_src text
../web/src/engine/mesh/painters.ts
71-    }
72-
73-    // When this value change, it means that the material has been invalidated and needs to be rebuilt.
74:    private get cacheKey(): string { //TODO: make a dedicated caching class (POP?).
75-        return ['texturePath']
76-            .map((key: string) => this.params[key])
77-            .join(' !! ');
...
85-        const res = new Uint8ClampedArray(w * h * 3);
86-        const color = new THREE.Color();
87-
88:        let idx = 0; //TODO: harmonize matrix layout everywhere, this is garbage.
89-        for (let j = h - 1; j >= 0; --j) {
90-            for (let i = 0; i < w; ++i) {
91-                const height = fun(i * sampling + halfcell, j * sampling + halfcell);

../web/src/noise/algorithms.ts
139-        // Dividing by the frequency dampens the impact of high frequencies, preventing them from
140-        // overwhelming the rest.
141-        // It makes the interactive parameters tweaking more intuitive.
142:        //TODO: Find a way to reduce the impact of very low frequencies (~.1), they make the
143-        // amplitude explode too much.
144-        const amplitude = octave.amplitude / frequency;
145-        const noise = octave.noise.make();
